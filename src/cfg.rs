...
impl Cfg {
    pub fn build(instructions: Vec<TackyInstr>) -> Self {
        ...
        for i in 0..num_blocks {
            ...
            match last {
                Some(TackyInstr::Return(_) | TackyInstr::Unreachable) => {
...
pub fn dead_store_elimination(
    ...
    let mut instr_live_after: InstrLiveAfter = HashMap::new();
    ...
    while let Some(block_id) = worklist.pop_front() {
        ...
        let (new_live_in, new_instr_live) =
            transfer_liveness(&cfg.blocks[block_id], &end_live, aliased_vars, static_vars);
...
fn meet_liveness(
    block: &BasicBlock,
    block_live_in: &HashMap<usize, HashSet<String>>,
    static_vars: &HashSet<String>,
) -> HashSet<String> {
...
fn transfer_liveness(
    ...
    aliased_vars: &HashSet<String>,
    _static_vars: &HashSet<String>,
) -> (HashSet<String>, InstrLiveAfter) {
...
fn get_instr_sources(instr: &TackyInstr) -> Vec<String> {
    ...
    match instr {
        ...
        | TackyInstr::DoubleToFloat { src, .. } => add_var(src, &mut srcs),
        TackyInstr::Store { src, dst_ptr } => {
            add_var(src, &mut srcs);
            add_var(dst_ptr, &mut srcs);
        }
